...
int _atoi(char *s)
{
	...
	while ((s[first_index] < '0' || s[first_index] > '9') && s[first_index] != '\0')
		first_index++;
	if (s[first_index] == '\0')
	{
		first_index--;
	...
	while (s[last_index] >= '0' && s[last_index] <= '9' && s[last_index] != '\0')
		last_index++;
	last_index--;
	while (last_index >= first_index && s[last_index] >= '0' && s[last_index] <= '9')
	{
